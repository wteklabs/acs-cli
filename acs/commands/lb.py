...
class Lb(Base):
    ...
    def open(self):
        ...
        if not port:
            self.logger.error(
                "Attempt to open an LB port without specifying '--port'")
        ...
        if protocol == "http":
            p = sub.Popen(
                ['azure', 'network', 'lb', 'probe', "create", "-o", str(port),
                 "-p", protocol, "-f", path, "-g", rg, "-l", lb_name, "-n",
                 probe_name], stdout=sub.PIPE, stderr=sub.PIPE)
        else:
            p = sub.Popen(
                ['azure', 'network', 'lb', 'probe', "create", "-o", str(port),
                 "-p", protocol, "-g", rg, "-l", lb_name, "-n", probe_name],
                stdout=sub.PIPE, stderr=sub.PIPE)

        output, errors = p.communicate()
        if errors:
            self.logger.error(
                "Error creating probe for LB rule:\n" + errors.decode("utf-8"))
        ...
        # Create an LB rule
        p = sub.Popen(
            ['azure', 'network', 'lb', 'rule', "create", "-g", rg, "-l",
             lb_name, "-n", rule_name, "-p", "tcp", "-f", str(port), "-b",
             str(port), "-o", backend_pool, "-a", probe_name], stdout=sub.PIPE,
            stderr=sub.PIPE)
        output, errors = p.communicate()
        if errors:
            self.logger.error(
                "Error creating LB rule:\n" + errors.decode("utf-8"))
            self.logger.error(
                "TODO: cleanup after failed LB port opening - delete probe that was created")
        ...
        priority = 350
        self.logger.warning(
            "FIXME: Currently nsg rule priority is set at 350, this will clash if we try to setup a second rule")
        p = sub.Popen(
            ['azure', 'network', 'nsg', "rule", "create", "-g", rg, "-a",
             nsg_name, "-p", "*", "-u", str(port), "-n", nsg_rule_name, "-y",
             str(priority)], stdout=sub.PIPE, stderr=sub.PIPE)
        output, errors = p.communicate()
        if errors:
            self.logger.error(
                "Error creating NSG rule:\n" + errors.decode("utf-8"))
            self.logger.error(
                "FIXME: cleanup after failed NSG rule creation - delete probe and LB rule that was created")
            return "Unable to create LB rule, see log for full details."
        self.logger.debug("Created NSG rule named " + nsg_rule_name)

        self.logger.info("Opened port " + str(
            port) + " in lb called " + lb_name + " using probe named " + probe_name)
